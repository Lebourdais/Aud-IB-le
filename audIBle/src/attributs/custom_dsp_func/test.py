...
acf,x_acf = multi_level_acf(sig,sr,space=(0.01,1,100))
...
plt_autocorr,=ax_bark2.plot(x_acf,acf[sig_displayed])
ax_bark2.set_title("Multi-level Windowed autocorr")
ax_bark2.set_xlabel("window size (s)")
def update(val):
    global plt_rms
    global plt_spec
    val = int(val)
    ...
    ax_bark.autoscale()
    ax_bark.relim()
    plt_autocorr.set_ydata(acf[val])
    ax_bark2.autoscale()
    ax_bark2.relim()
    #plt_autocorr.set_data(x_acf[val])
    
    #ax_bark2.get_autoscalex_on()
