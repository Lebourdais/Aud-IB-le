...
def train(config, conf_id, seed):
    ...
    # load model and its pretrained audio sparse autoencoder
    state_dict_pth = os.path.join(config["model"]["asae_exp_path"], "best_model.pth")
    state_dict = torch.load(state_dict_pth)
    asae_cfg = os.path.join(config["model"]["asae_exp_path"], "config.json")
    ...
    lambda_ce, lambda_spec = config["optim"]["loss_weights"]
    ...
    for epoch in range(config["optim"]["epochs"]):
        ...
        for i, (inputs, labels) in enumerate(tqdm.tqdm(dataloader)):
            ...
            # Forward pass
            y_hat, spec_reconstruct, spec, hidden = model(inputs)
            ...
            loss = lambda_ce * loss_ce + lambda_spec * loss_spec #+ lambda_sae * loss_sae 
            ...
            writer.add_scalar('train/CE', loss_ce.item(), epoch * len(dataloader) + i)
        ...
        with torch.no_grad():
            ...
            writer.add_scalar('valid/CE', valid_loss_ce/len_valid, epoch )
...
if __name__ == "__main__":
    ...
    from audIBle.config.asae_classif_cfg import conf, common_parameters
